...
contract vote{
    ...
    function calculateCurrentCarbonCost() public{
        ...
        //为什么索引为uint
        int carbon_quota = (suggestAllowance[sender.level]/100)*sum;
        ...
        int carbon_cost_per_week = carbon_cost/sum; //小数？
        int residual_carbon_quota = (carbon_quota - carbon_cost)/1000; //小数？
        sender.carbon_cost_per_week = carbon_cost_per_week;
    ...
    function getDecreasedAllowancePercentage() public{
        ...
        if(sender.target_carbon_allowance == 0){
            ...
            int a = (suggestAllowance[sender.level]/100) - voting_carbon_allowance;
            int b = a*100/(suggestAllowance[sender.level]/100);
            ...
            sender.decreasedAllowancePercentage_ = ((sender.target_carbon_allowance - voting_carbon_allowance)/sender.target_carbon_allowance)*100;
        }
    ...
    function ExpectCarbonAllowance() public{
        ...
        int next_week_carbon_allowance;
        int decreased_allowance;
        int residual_allowance;
        //第一次投票，没有之前的数据
        if(sender.target_carbon_allowance == 0){
            //得到下周的carbon_allowance
            next_week_carbon_allowance = (suggestAllowance[sender.level]/100)*(1-(decreasedAllowancePercentage_/100));
            decreased_allowance = (suggestAllowance[sender.level]/100) - next_week_carbon_allowance;
            residual_allowance = (suggestAllowance[sender.level]/100) - sender.carbon_cost_per_week;
        }else{
            //并非第一次投票，那么需要target_carbon_allowance, 还有已经存在的governanceToken来计算这一周的碳消耗
            next_week_carbon_allowance = (sender.target_carbon_allowance)*(1-(decreasedAllowancePercentage_/100));
        ...
        //得到新的governanceToken
        sender.governanceToken += (residual_allowance + decreased_allowance);
